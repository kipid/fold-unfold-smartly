...
export function activate(context: vscode.ExtensionContext) {
  ...
  vscode.window.onDidChangeActiveTextEditor((editor) => {
    if (editor) {
      ...
      const upwardDecorations: vscode.DecorationOptions[] = []; // 위쪽 방향 기호를 위한 배열 "≻"
      const downwardDecorations: vscode.DecorationOptions[] = []; // 아래쪽 방향 기호를 위한 배열 "≺"
      const upwardAvailableDecorations: vscode.DecorationOptions[] = []; // 위쪽에서 fold 가능한 상태 기호 배열 "∨"
      const downwardAvailableDecorations: vscode.DecorationOptions[] = []; // 아래쪽에서 fold 가능한 상태 기호 배열 "∧"

      // 각 줄을 순회하며 기호를 설정
      for (let i = 0; i < lines.length; i++) {
        const currentIndentation = lines[i].match(/^\s*/)?.[0]!; // 현재 줄의 indentation 가져오기
    ...
    selections.forEach((selection) => {
      ...
      if (res) {
        vscode.window.showInformationMessage(
          `Folding lines from ${res.initialRange.initialLine + 1} to ${
            res.lastRange.lastLine + 1
          }`
        );
        ...
        for (
          let i = res.initialRange.initialLine;
          i <= res.lastRange.lastLine;
          i++
        ) {
          rangesToFold.push(
