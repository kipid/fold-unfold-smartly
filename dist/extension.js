...
console.log("SmartFold extension loaded");
...
// --- Decoration Types ---
let foldDownAvailableDecoration;
let foldUpAvailableDecoration;
let foldDownFoldedDecoration;
let foldUpFoldedDecoration;
// --- Helper Functions ---
/**
 * Finds the foldable range starting from a given line.
 */
function findFoldableRange(lines, startLineIndex) {
    console.log(`[SmartFold DBG] findFoldableRange called for line: ${startLineIndex + 1}`); // Debug log
    ...
    console.log(`[SmartFold DBG] Start Indentation: '${startIndentation}' (length ${startIndentation.length})`); // Debug log
    let endLineIndex = -1;
    ...
    while (k < lines.length) {
        ...
        if (!currentIndentationMatch)
            break; // Should not happen
        const currentIndentation = currentIndentationMatch[0];
        console.log(`[SmartFold DBG] Checking line ${k + 1}: Indent '${currentIndentation}' (length ${currentIndentation.length}), Content: '${currentLine.trim()}'`); // Debug log
        // Check if it's a child or sibling with content
        if (currentIndentation.length > startIndentation.length &&
            !/^\s*$/.test(currentLine)) {
        ...
        else if (currentIndentation.length <= startIndentation.length &&
            !/^\s*$/.test(currentLine)) {
            console.log(`[SmartFold DBG]   Found sibling/parent with content. Breaking.`); // Debug log
            break;
        }
            ...
            console.log(`[SmartFold DBG]   Equal/less indentation before child. Breaking.`); // Debug log
            break;
        }
    }
    if (foundChild && endLineIndex !== -1) {
        console.log(`[SmartFold DBG] Found range: ${startLineIndex + 1} to ${endLineIndex + 1}`); // Debug log
        return { start: startLineIndex, end: endLineIndex };
    }
    console.log(`[SmartFold DBG] No foldable range found starting at line ${startLineIndex + 1}`); // Debug log
    return null;
}
/**
...
function updateDecorations(editor) {
    ...
    const isLineFolded = (line) => {
        return currentlyFoldedRanges.some((folded) => line > folded.start && line < folded.end);
    };
    const getFoldStartLine = (line) => {
        const fold = currentlyFoldedRanges.find((folded) => line > folded.start && line < folded.end);
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (rangeInfo) {
            ...
            console.log(`[SmartFold] Found foldable range for line ${i + 1}: ${start + 1}-${end + 1}`); // Log was already uncommented
            // --- Determine Folded State ---
            // Check if the line *immediately after* the start is visible.
            let isRangeActuallyFolded = false;
            if (start + 1 <= end) {
                isRangeActuallyFolded = !editor.visibleRanges.some((vr) => vr.start.line <= start + 1 && vr.end.line >= start + 1);
            }
            console.log(`[SmartFold] Line: ${start + 1}, Is Range Folded? ${isRangeActuallyFolded}`); // Log was already uncommented
            // --- Apply Decorations ---
            ...
            if (isRangeActuallyFolded) {
                // Folded state
                console.log(`[SmartFold] Applying Folded icons for ${start + 1}-${end + 1}`); // Log was already uncommented
                ...
                console.log(`[SmartFold] Applying Available icons for ${start + 1}-${end + 1}`); // Log was already uncommented
    ...
    // Apply decorations
    console.log(`[SmartFold] Applying decorations: downAvailable=${decorations.downAvailable.length}, upAvailable=${decorations.upAvailable.length}, downFolded=${decorations.downFolded.length}, upFolded=${decorations.upFolded.length}`); // Log was already uncommented
...
function activate(context) {
    ...
    const iconPath = (iconName) => vscode.Uri.file(context.asAbsolutePath(`icons/${iconName}.svg`));
    ...
    // --- Commands ---
    const foldCommand = vscode.commands.registerTextEditorCommand("fold-unfold-smartly.fold", (editor, edit) => {
        ...
        if (rangeInfo) {
            ...
            vscode.window.showInformationMessage("No foldable range starting at this line.");
        }
    });
    const unfoldCommand = vscode.commands.registerTextEditorCommand("fold-unfold-smartly.unfold", (editor, edit) => {
        ...
        const isPotentiallyFoldedStart = !editor.visibleRanges.some((vr) => vr.start.line <= line + 1 && vr.end.line >= line + 1);
        if (isPotentiallyFoldedStart) {
            ...
            const isLineHidden = !editor.visibleRanges.some((vr) => vr.contains(editor.selection.active));
            if (isLineHidden) {
                ...
                vscode.window.showInformationMessage("No folded range to unfold at this line.");
            }
        }
    });
    // --- Event Listeners ---
    ...
    const foldingChangeListener = vscode.window.onDidChangeTextEditorVisibleRanges((event) => {
        updateDecorations(event.textEditor);
    });
    // --- Subscriptions ---
    context.subscriptions.push(foldCommand, unfoldCommand, activeEditorListener, textChangeListener, foldingChangeListener, // Add the new listener
    foldDownAvailableDecoration, foldUpAvailableDecoration, foldDownFoldedDecoration, foldUpFoldedDecoration);
...
function deactivate() {
    ...
    foldDownAvailableDecoration === null || foldDownAvailableDecoration === void 0 ? void 0 : foldDownAvailableDecoration.dispose();
    foldUpAvailableDecoration === null || foldUpAvailableDecoration === void 0 ? void 0 : foldUpAvailableDecoration.dispose();
    foldDownFoldedDecoration === null || foldDownFoldedDecoration === void 0 ? void 0 : foldDownFoldedDecoration.dispose();
    foldUpFoldedDecoration === null || foldUpFoldedDecoration === void 0 ? void 0 : foldUpFoldedDecoration.dispose();
}
